...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = self.conv1(x)  # [2,64,75,75]
        out = self.bn1(out)  # [2,64,75,75]
        out = self.relu(out)  # [2,64,75,75]

        out = self.conv2(out)  # [2,64,75,75]
        out = self.bn2(out)  # [2,64,75,75]
        out = self.relu(out)  # [2,64,75,75]

        out = self.conv3(out)  # [2,256,75,75]
        out = self.bn3(out)  # [2,256,75,75]

        if self.downsample is not None:
            identity = self.downsample(x)
        out += identity  # [2,256,75,75]
        out = self.relu(out)  # [2,256,75,75]
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.conv1(x)  # [2,16,150,150]
        x = self.bn1(x)  # [2,16,150,150]
        x = self.relu(x)  # [2,64,150,150]
        x = self.maxpool(x)  # [2,64,75,75]

        x = self.layer1(x)  # [2,256,75,75]
        x = self.layer2(x)  # [2,512,38,38]
        x = self.layer3(x)  # [2,1024,19,19]
        x = self.layer4(x)  # [2,2048,10,10]

        x_1 = self.AFNB(x)
        x_1 = self.avgpool(x_1)
        x_1 = torch.flatten(x_1, 1)
        x = self.avgpool(x)  # [2,2048,1,1]
        x = torch.flatten(x, 1)  # [2,2048]
        x = self.fc(x)  # [2,1000]
...
class AFNB(nn.Module):
    ...
    def forward(self, x):
        ...
        xxx = self.conv_theta_spp(self.conv_theta(x))  # [2,32,75,75]
        # [N, H * W, C/2]
        x_theta = self.conv_theta_spp(self.conv_theta(x)).view(b, self.sample_dim, -1)
