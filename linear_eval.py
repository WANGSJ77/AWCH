...
def main(args):
    ...
    # train_data = MyCustomDataset(root_path="./Data/UCM/train.txt",
    #                              transform=get_aug(train=True, train_classifier=False, **args.aug_kwargs))
    # test_data = MyCustomDataset(root_path="./Data/UCM/test.txt",
    #                             transform=get_aug(train=False, train_classifier=False, **args.aug_kwargs))
    # train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.train.batch_size, shuffle=True,
    #                                            num_workers=0, pin_memory=True, drop_last=True)
    # test_loader = torch.utils.data.DataLoader(test_data, batch_size=args.train.batch_size, shuffle=False, num_workers=0,
    #                                           pin_memory=True)
    ...
    for epoch in global_progress:
        ...
        for idx, (images, labels) in enumerate(local_progress):
            ...
            loss = F.cross_entropy(preds, labels.long().to(args.device))
...
if __name__ == "__main__":
    # --data_dir ./Data/ --log_dir ./logs/ -c configs/simclr_ucm.yaml --ckpt_dir ./cache/ --eval_from ./cache/simclr-UCM-experiment_0320220344.pth --hide_progress
    main(args=get_args())
