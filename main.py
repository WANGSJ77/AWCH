...
def main(device, args):
    # 已有数据集：--data_dir ./Data/ --log_dir ./logs/ -c configs/simclr_ucm.yaml --ckpt_dir ./cache/ --hide_progress
    ...
    # Save checkpoint
    model_path = os.path.join(args.ckpt_dir,f"{args.name}_{datetime.now().strftime('%y%m%d%H%M%S')}.pth")# datetime.now().strftime('%Y%m%d_%H%M%S')
    torch.save({
        'epoch': epoch+1,
